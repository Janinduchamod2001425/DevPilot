...
export class DeploymentsService {
    ...
    async create(
        ...
        const project =
            await this.prismaService.client.project.findUnique({
                where: {
                    id: dto.projectId,
                },
            });
        ...
        if (!project) {
            throw new NotFoundException(
                `Project ${dto.projectId} was not found`,
            );
        }

        const deployment =
            await this.prismaService.client.deployment.create({
                data: {
    ...
    async findOne(id: string): Promise<Deployment> {
        const deployment =
            await this.prismaService.client.deployment.findUnique({
                where: {
                    id,
                },
            });
